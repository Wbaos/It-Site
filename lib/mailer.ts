...
export async function sendCustomerOrderConfirmationEmail(params: {
  ...
  const to = String(order?.email || order?.contact?.email || "").trim();
  if (!to || to.toLowerCase() === "unknown") {
    return { ok: false as const, skipped: true as const, reason: "Missing customer email" };
  }
  ...
  const replyTo = String(process.env.REPLY_TO_EMAIL || "").trim() || undefined;
  const baseUrl = String(process.env.NEXT_PUBLIC_BASE_URL || "").trim();
  ...
  const html = `
    ...
    <p>We’ve received your order and will follow up if we need any more details.</p>
    ...
    <p><b>Scheduled:</b> ${escapeHtml(schedule?.date || "")} ${escapeHtml(schedule?.time || "")}</p>
    <p><b>Address:</b><br />
      ${escapeHtml(address?.street || "")}${address?.street ? "<br />" : ""}
      ${escapeHtml(address?.city || "")}${address?.city ? ", " : ""}${escapeHtml(address?.state || "")} ${escapeHtml(address?.zip || "")}
    </p>
    ${baseUrl ? `<hr /><p><a href="${escapeHtml(baseUrl)}/account" target="_blank" rel="noreferrer">View your order in your account</a></p>` : ""}
    <p style="color:#64748b; font-size:12px;">If you have questions, reply to this email.</p>
    <p style="color:#64748b; font-size:12px;">Source: ${escapeHtml(source)}</p>
  `;
  ...
  if (process.env.RESEND_API_KEY) {
    try {
      ...
      logger.error("Customer order confirmation failed (Resend)", err, { to, source });
    }
  }
